...
import { WrongQuestion, WeeklyProgress, GuideAnalytics } from '@/interfaces/test';
...
export default function DashboardPage() {
  ...
  const { data: studyHours } = useSWR(userId ? ENDPOINTS.studyHours(userId) : null, fetcher);
  const { data: testAnalytics } = useSWR(userId ? ENDPOINTS.testAnalytics(userId) : null, fetcher);
  ...
  const studyGuides = useMemo(() => {
    ...
    return studyGuidesResponse.study_guides.map((guide: { study_guide_id: string; title: string }) => ({
  ...
  const { data: guideAnalytics } = useSWR(
    userId && selectedGuide?.id ? ENDPOINTS.guideAnalytics(userId, selectedGuide.id) : null,
    fetcher
  );
  ...
  const handlePreviousGuide = () => {
    setSelectedGuideIndex((prev) => (prev === 0 ? (studyGuides?.length || 1) - 1 : prev - 1));
  };

  const handleNextGuide = () => {
    setSelectedGuideIndex((prev) => (prev === (studyGuides?.length || 1) - 1 ? 0 : prev + 1));
  };
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <Tabs defaultValue="overall" className="mb-8">
              <TabsList className="flex w-full border-b border-gray-200 mb-4 p-0 bg-transparent space-x-4">
                ...
                <TabsTrigger value="guide-specific">Guide-Specific Stats</TabsTrigger>
              </TabsList>
                ...
                <motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  <Card className="bg-white shadow-lg hover:shadow-xl transition-shadow duration-300">
                    <CardContent className="p-6">
                      ...
                      <CardTitle className="text-lg font-semibold">Total Study Time</CardTitle>
                      <p className="text-2xl font-bold">{studyHours?.total_hours || 'N/A'} hours</p>
                    </CardContent>
                  </Card>
                  ...
                  <Card className="bg-white shadow-lg hover:shadow-xl transition-shadow duration-300">
                    <CardContent className="p-6">
                      ...
                      <p className="text-2xl font-bold">{testAnalytics?.average_score.toFixed(2) || 'N/A'} pts</p>
                    </CardContent>
                  </Card>
                  ...
                  <Card className="bg-white shadow-lg hover:shadow-xl transition-shadow duration-300">
                    <CardContent className="p-6">
                      ...
                      <CardTitle className="text-lg font-semibold">Tests Taken</CardTitle>
                      <p className="text-2xl font-bold">{testAnalytics?.total_tests || 'N/A'}</p>
                    </CardContent>
                  </Card>
                </motion.div>
              </TabsContent>
